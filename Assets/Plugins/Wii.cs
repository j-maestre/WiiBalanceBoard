...
public class Wii : MonoBehaviour {
    ...
    public static void SetLEDs(int remote, bool LED1, bool LED2, bool LED3, bool LED4) {
        ...
        if (LED1 != theLEDs[remote][0] || LED2 != theLEDs[remote][1] || LED3 != theLEDs[remote][2] ||
            LED4 != theLEDs[remote][3]) {
    ...
    public static Vector2 GetIRPosition(int remote) {
        ...
        if ((currentStates[remote].wiiAccelZ + previousStates[remote].wiiAccelZ) / 2.0f > 0) { //right side up
            ...
            rotation = (rotation + (Mathf.Atan(previousStates[remote].wiiAccelX / previousStates[remote].wiiAccelZ) /
                                    3.0f)) / 2.0f;
            ...
            rotation =
                (rotation + ((3 + Mathf.Atan(previousStates[remote].wiiAccelX / previousStates[remote].wiiAccelZ)) /
                             3.0f)) / 2.0f;
        } else {
    ...
    public static void CalibrateMotionPlus(int remote) {
        ...
        setMotionPlusCalibration(remote, currentStates[remote].roll, currentStates[remote].yaw,
            currentStates[remote].pitch);
    }
    ...
    public static Vector2 GetClassicAnalogStick(int remote, string s) {
        ...
        if (currentStates[remote].expType != 2 || float.IsNaN(currentStates[remote].expFloat1) ||
            float.IsNaN(currentStates[remote].expFloat3))
            return Vector2.zero;
        switch (s) {
            case "LEFT":
                return new Vector2(currentStates[remote].expFloat1, currentStates[remote].expFloat2);
        ...
        if (currentStates[remote].expType == 2) {
            switch (s) {
                case "CLASSICL":
                    ...
                    return (currentStates[remote].expFloat5 - classicAnalogButtonMin) /
                           (classicAnalogButtonMax - classicAnalogButtonMin);
                case "CLASSICR":
                    if (isVirtual[remote])
                        return currentStates[remote].expFloat6;
                    return (currentStates[remote].expFloat6 - classicAnalogButtonMin) /
                           (classicAnalogButtonMax - classicAnalogButtonMin);
                default:
                    return 0.0f;
            }
    ...
    public static Vector2 GetWiiUProAnalogStick(int remote, string s) {
        ...
        if (currentStates[remote].expType != 7 || float.IsNaN(currentStates[remote].expFloat1) ||
            float.IsNaN(currentStates[remote].expFloat3))
            return Vector2.zero;
        ...
        switch (s) {
            case "LEFT":
                return new Vector2(
                    (currentStates[remote].expFloat1 - wiiUProLeftCalibs[remote].x) / uProStickNormalizer,
                    (currentStates[remote].expFloat2 - wiiUProLeftCalibs[remote].y) / uProStickNormalizer);
            case "RIGHT":
                return new Vector2(
                    (currentStates[remote].expFloat3 - wiiUProRightCalibs[remote].x) / uProStickNormalizer,
                    (currentStates[remote].expFloat4 - wiiUProRightCalibs[remote].y) / uProStickNormalizer);
            default:
                print("Unsupported Analog Stick Type " + s);
                return Vector2.zero;
        }
    ...
    public struct State {
        ...
        [MarshalAs(UnmanagedType.Struct)] [FieldOffset(96)]
        public IRData rawIR1;

        [MarshalAs(UnmanagedType.Struct)] [FieldOffset(102)]
        public IRData rawIR2;

        [MarshalAs(UnmanagedType.Struct)] [FieldOffset(108)]
        public IRData rawIR3;

        [MarshalAs(UnmanagedType.Struct)] [FieldOffset(114)]
        public IRData rawIR4;
    ...
    void FixedUpdate() {
        ...
        if (searching || Application.isEditor
        ) //(because the extension could also cause new ones to be discovered at any time.)
        {
        ...
        for (int i = 0; i < highestNumberedPlayer; i++) {
            if (wiiRemotesActive[i] && !isVirtual[i]) {
                ...
                if (automaticallyCalibrateMotionPlus && currentStates[i].motionPlusAvailable &&
                    !currentStates[i].motionPlusCalibrated) //motionplus needs calibratin'
                {
                    ...
                    if (currentStates[i].wiiAccelZ < -.8 && wiggles < .1f) {
                        ...
                        if (motionPlusCalib[i][0] > calibrationBuffer) //let it settle before starting
                        {
                            ...
                            motionPlusCalib[i][1] +=
                                (currentStates[i].roll / (float) calibrationCounterLimit); //rollzero
                            motionPlusCalib[i][2] += (currentStates[i].yaw / (float) calibrationCounterLimit); //yawZero
                            motionPlusCalib[i][3] +=
                                (currentStates[i].pitch / (float) calibrationCounterLimit); //pitchZero
                        }
                        ...
                        if (motionPlusCalib[i][0] >= calibrationCounterLimit + calibrationBuffer) //taken enough samples
                        {
                            ...
                            setMotionPlusCalibration(i, motionPlusCalib[i][1], motionPlusCalib[i][2],
                                motionPlusCalib[i][3]);
